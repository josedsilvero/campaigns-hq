...
AS SELECT f.campaign_id AS facebook_id,
    ...
    trunc(f.spend /nullif (f.purchase_value,0),2) as ecpa,
    trunc((c.revenue_events*100)/nullif(f.purchase_value,0),2) as conv_valid,
    trunc((c.revenue_events*100)/nullif(c.lander_visitors,0),2) as ctr_serp,
    trunc(c.revenue / nullif(c.revenue_events,0),2) as avg_rpc
